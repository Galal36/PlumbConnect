...
class LocationViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        """
        Allow public read access to locations, but require authentication for write operations
        """
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        ...
        if self.action in ['plumbers']:
            return [permissions.AllowAny()]
        #List requires authentication for complaints system
    ...
    def plumbers(self, request):
        """
        Get all plumbers (users with role='plumber')
        """
        plumbers = User.objects.filter(role='plumber', status='active').select_related('location')
        serializer = self.get_serializer(plumbers, many=True)
